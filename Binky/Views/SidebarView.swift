...
struct SidebarView: View {
    ...
    /// Opens Settings to a tab; prefer ``SettingsLink`` + ``PreferencesTab/stagePendingTab(_:)`` from window UI (SwiftUI warns on `openSettings()`).
    ...
    private var simpleModeExtras: some View {
        VStack(alignment: .leading, spacing: 8) {
            VStack(alignment: .leading, spacing: 8) {
                ...
                .font(.system(size: 11))
                .accessibilityHint(String(localized: "Binky picks WebP, AVIF, or another output format per image — new files, not a same-format JPEG squeeze. Turn off Quick choices to configure manually.", comment: "VoiceOver: simple mode automatic toggle."))
        ...
        Group {
            if expandedOutcomeMap {
                ...
                simpleModeOutcomeMapStack
                    ...
                    .accessibilityLabel(
                        String(
                            localized: "What to expect, collapsed. How Binky handles photos, documents, video, and saved files. Activate the header to show friendly and technical details.",
    ...
    private var simpleModeOutcomeMapStack: some View {
        VStack(alignment: .leading, spacing: 6) {
            ...
            if !expandedOutcomeMap {
                Text(String(localized: "How Binky handles photos, documents, video, and saved files — expand for details.", comment: "Collapsed teaser under What to expect header."))
                    .font(.system(size: 9))
                    .foregroundStyle(.tertiary)
                    .fixedSize(horizontal: false, vertical: true)
    ...
    private var pdfsContent: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            if prefs.pdfOutputMode == .flattenPages {
                ...
                if prefs.smartQuality {
                    settingsHelperText(String(localized: "Binky picks a tier from each document. The choice below is the manual fallback if analysis fails.", comment: "Sidebar PDF: smart flatten helper."))
                        .transition(.asymmetric(
                            insertion: .move(edge: .top).combined(with: .opacity.animation(.easeInOut(duration: 0.15).delay(0.1))),
                            removal:   .move(edge: .top).combined(with: .opacity.animation(.easeIn(duration: 0.08)))
                        ))
                ...
                if prefs.pdfMaxFileSizeEnabled,
                   PDFQuality.flattenUIShowableTiers(maxFileSizeEnabled: true, pdfMaxFileSizeKB: prefs.pdfMaxFileSizeKB).count < PDFQuality.allCases.count {
                    settingsHelperText(String(localized: "Tighter max-size targets only list lower starting tiers; Binky still steps down through the chain if needed.", comment: "Sidebar PDF: max size limits tier chips helper."))
                        .transition(.asymmetric(
                            insertion: .move(edge: .top).combined(with: .opacity.animation(.easeInOut(duration: 0.15).delay(0.1))),
                            removal:   .move(edge: .top).combined(with: .opacity.animation(.easeIn(duration: 0.08)))
                        ))
    ...
    Text(label)
        ...
        .background(
            RoundedRectangle(cornerRadius: 6, style: .continuous)
                .fill(active ? AnyShapeStyle(binkyGradient) : AnyShapeStyle(Color.primary.opacity(0.08)))
        )
        .contentShape(Rectangle())
}
