...
final class DiagnosticsReporter: NSObject, ObservableObject {
    ...
    private var appSupportBinkyURL: URL {
        FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask)[0]
            .appendingPathComponent("Binky", isDirectory: true)
    }

    private var sentinelURL: URL {
        appSupportBinkyURL.appendingPathComponent(Self.sentinelName)
    }

    private var diagnosticsJSONURL: URL {
        appSupportBinkyURL.appendingPathComponent(Self.diagnosticsFileName)
    }
    ...
    func startMonitoring() {
        ...
        try? FileManager.default.createDirectory(at: appSupportBinkyURL, withIntermediateDirectories: true)
    ...
    /// directly so this reporter doesn't depend on the `BinkyPreferences` instance lifecycle.
    ...
    func postCrashEmailURL(for report: CrashReport) -> URL {
        ...
        return Self.emailURL(subject: String(localized: "Crash report — Binky", comment: "Email subject for crash report."), extraBody: extraBody)
    }
    ...
    func postCrashGitHubURL(for report: CrashReport) -> URL {
        ...
        return Self.githubIssueURL(title: String(localized: "Crash — Binky", comment: "GitHub issue title for crash report."), extraBody: extraBody)
    }
    ...
    static func githubIssueURL(title: String, extraBody: String = "") -> URL {
        let body =
            ...
            + extraBody
            + "\n\n## Steps to reproduce\n\n1. \n\n"
        var components = URLComponents(string: "https://github.com/heyderekj/binky/issues/new")!
        components.queryItems = [
            URLQueryItem(name: "title", value: title),
            URLQueryItem(name: "body", value: body),
        ]
        return components.url ?? URL(string: "https://github.com/heyderekj/binky/issues/new")!
    }
