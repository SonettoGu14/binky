...
enum SortDisposition: String, Codable, Sendable {
    case moved, kept
    case skippedTransient, skippedStableCheckTimeout, skippedError
}
...
struct SortBatchOutcome: Identifiable, Equatable, Codable, Sendable {
    ...
    var skippedCount: Int {
        entries.count(where: {
            ...
            case .skippedTransient, .skippedStableCheckTimeout, .skippedError: return true
...
private enum FinderTagApplicator {
...
final class DownloadsSortOrchestrator {
    ...
    func sort(files urls: [URL], prefs: BinkyPreferences) async -> SortBatchOutcome {
        ...
        for raw in urls {
            ...
            let category = FileClassification.categorize(url: standardized)
            let destinationDir = StarterBuckets.directory(for: category, root: root)
            ...
            let target = DownloadsSortOrchestrator.uniquify(destinationDirectory: destinationDir, preferredFilename: standardized.lastPathComponent)
            ...
            do {
                ...
                if prefs.assignFinderTagsOnSortEnabled {
                    FinderTagApplicator.merge(prefs.sortTagComposition(forCategory: category), onto: target)
                }
                ...
                rows.append(entry(path: standardized.path,
                                  ...
                                  category: category,
                                  disposition: .moved,
                                  reason: localizedMoveReason(category)))
    ...
    private func entry(path: String, dest: String?, category: FileSortCategory,
                       disposition: SortDisposition, reason: String) -> SortBatchEntry {
        .init(id: UUID(), sourcePath: path, destinationPath: dest, category: category, disposition: disposition, reason: reason)
    }

    private func localizedMoveReason(_ cat: FileSortCategory) -> String {
        String.localizedStringWithFormat(String(localized: "Moved into “%@”.", comment: "Sort audit reason; formatted folder name."), cat.downloadsSubfolder)
    }
