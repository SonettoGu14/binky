...
private func heicTranscodeToPNGIfNeeded(source: URL) throws -> URL {
    ...
    let tmpURL = FileManager.default.temporaryDirectory
        .appendingPathComponent("binky_heic_\(UUID().uuidString)")
...
private func resizeImageMaxWidthUsingImageIO(source: URL, maxWidth: Int) throws -> URL {
    ...
    let tmpURL = FileManager.default.temporaryDirectory
        .appendingPathComponent("binky_resize_io_\(UUID().uuidString)")
        .appendingPathExtension("png")
...
actor CompressionService {
    ...
    private func compressToTargetSize(
        ...
        if format == .webp, content == .graphic {
            ...
            while nlLo <= nlHi {
                ...
                let tmp = fm.temporaryDirectory
                    .appendingPathComponent("binky_nl\(mid)_\(UUID().uuidString)")
        ...
        while lo <= hi {
            ...
            let mid = (lo + hi) / 2
            let tmp = fm.temporaryDirectory
                .appendingPathComponent("binky_q\(mid)_\(UUID().uuidString)")
                .appendingPathExtension(format.outputExtension)
        ...
        case .preserveStructure:
            ...
            if let qpdfBin = qpdfBinaryURL() {
                ...
                for (idx, step) in steps.enumerated() {
                    let qpdfTmp = fm.temporaryDirectory.appendingPathComponent("binky_qpdf_\(UUID().uuidString).pdf")
            ...
            if resolutionDownsampling, fm.fileExists(atPath: effOut.path),
               let structureDoc = PDFDocument(url: effOut) {
                let dsURL = fm.temporaryDirectory.appendingPathComponent("binky_pdf_ds_\(UUID().uuidString).pdf")
