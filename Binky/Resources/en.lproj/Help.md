...
Binky keeps your **Downloads inbox** tidy: it watches for finished files, moves them into sensible folders, optionally adds Finder tags, and shows a clear **move / review summary** so you always know what happened.
...
**Sort with Binky** is in Finder **Services** (and as a **Quick Action**). Turn it on under **System Settings → Keyboard → Keyboard Shortcuts → Services**.

- **Folders:** Right-click a folder and choose **Sort with Binky** — Binky treats that folder as a one-shot inbox. It sorts the **files at the top level** into the usual subfolders (`Images`, `Documents`, etc.) **inside** the folder you picked. Nested subfolders are left alone. Works anywhere on disk, not only the watched inbox.
- **Single files:** Same routing rules as the main window, but the file has to live **inside** the inbox you’re watching — anything outside that is ignored.

Dropping **folders** or **files** onto Binky’s **Dock icon** follows the same rules (folders expand; loose files stay inbox-only).

## Shortcuts app

Binky exposes a **Sort files** intent so you can automate sorting from the Shortcuts app — same routing rules as the main window.
