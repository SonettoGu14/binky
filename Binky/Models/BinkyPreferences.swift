...
enum FilenameHandling: String, CaseIterable, Identifiable {
    ...
    var displayName: String {
        switch self {
        case .appendSuffix:  return "Append suffix (default: -binky)"
...
final class BinkyPreferences: ObservableObject {
    ...
    @AppStorage("customSuffix")         var customSuffix: String = "-binky"
    ...
    /// Off by default to keep Binky's "no telemetry" promise — even when on, nothing
    ...
    /// When true, merges Finder tags after each automated sort.
    ...
    func setShortcut(_ shortcut: CustomShortcut, for action: ShortcutAction) {
        ...
        if action == .pasteClipboard {
            NotificationCenter.default.post(name: .binkyGlobalPasteHotkeyChanged, object: nil)
        }
    }
    ...
    /// Default backup folder when the user hasn't picked one: `~/Pictures/Binky Originals`.
    func defaultOriginalsBackupFolderURL() -> URL {
        ...
        return pictures.appendingPathComponent("Binky Originals", isDirectory: true)
    }
    ...
    func outputFilenameSummaryLine() -> String {
        switch filenameHandling {
        case .appendSuffix:
            return String(localized: "Adds “-binky” before the extension", comment: "Output summary: filename handling.")
        case .replaceOrigin:
