...
struct CompressionPreset: Codable, Identifiable {
    ...
    /// Suffix or template when `collisionNamingStyleRaw` is `custom` (same rules as `BinkyPreferences.collisionCustomPattern`).
    ...
    /// Smart Quality: auto-grayscale flatten for detected monochrome scans (mirrors `BinkyPreferences.pdfAutoGrayscaleMonoScans`).
    var pdfAutoGrayscaleMonoScans: Bool
    /// Experimental qpdf passes when PDF output is preserve (mirrors `BinkyPreferences.pdfPreserveExperimental`).
    ...
    init(name: String, from prefs: BinkyPreferences, format: CompressionFormat) {
        self.id = UUID()
        self.name = name
        self.format = format
    ...
    init(from decoder: Decoder) throws {
        ...
        customSuffix = try c.decodeIfPresent(String.self, forKey: .customSuffix) ?? "-binky"
...
// MARK: - Output paths (same rules as `BinkyPreferences`, with per-preset destination)
...
extension CompressionPreset {
    ...
    func destinationDirectory(for source: URL, globalPrefs: BinkyPreferences, isFromURLDownload: Bool = false) -> URL {
    ...
    func outputURL(for source: URL, format: CompressionFormat, globalPrefs: BinkyPreferences, isFromURLDownload: Bool = false) -> URL {
        ...
        case .appendSuffix: out = stem + "-binky"
        case .replaceOrigin: out = stem
        case .customSuffix: out = stem + (customSuffix.isEmpty ? "-binky" : customSuffix)
        }
    ...
    func outputURL(for source: URL, mediaType: MediaType, globalPrefs: BinkyPreferences, isFromURLDownload: Bool = false) -> URL {
        switch mediaType {
        case .image:
            let dir = destinationDirectory(for: source, globalPrefs: globalPrefs, isFromURLDownload: isFromURLDownload)
            let stem = source.deletingPathExtension().lastPathComponent
